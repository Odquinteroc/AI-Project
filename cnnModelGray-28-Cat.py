...
output_path = f"/content/drive/MyDrive/IA_proyect/output/grayscale_without_augmentation/1_model_28_categories/"
...
for fruit in fruits:
  ...
  for category in categories:
    ...
    for img_name in files_images:
      ...
      image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) # Transform the image to grayscale
      image = image.reshape(image_size, image_size, 1) # Change shape to 128, 128, 1
...
# Create a DataFrame with the columns 'categoria' and 'label'
...
modelCNN_color = tf.keras.models.Sequential([
  tf.keras.layers.Input(shape=(image_size, image_size, 1)),  # Explicitly define input shape
...
modelCNN_color.fit(X_train, y_train, batch_size= 500, epochs=50,
...
# Save the model
modelCNN_color.save(f"{output_path}modelCNN_gray.h5")
